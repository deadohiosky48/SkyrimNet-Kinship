...
namespace Kinship::Diagnostics {
    ...
    std::string DumpPackages() {
        ...
        for (const auto& c : Store::Children()) {
            ...
            //
            // BOTH KEYWORDS, because the experiment uses a different one.
            // LinkCustom02 is what the shipped package resolves against;
            // DLC1LinkSandbox is what Dawnguard's wider-radius family uses. A
            // child switched to the experiment has the second and not the
            // first, and reporting only the first would read as "no anchor"
            // for exactly the children being tested.
            //
            // Dawnguard's keyword is looked up through its own file rather
            // than by a hardcoded 0x02... - the 02 prefix is only correct when
            // Dawnguard happens to be the third master loaded.
            struct Anchor { const char* label; RE::BGSKeyword* kw; };
            auto* dlc1 = RE::TESDataHandler::GetSingleton()
                             ? RE::TESDataHandler::GetSingleton()
                                   ->LookupForm<RE::BGSKeyword>(0x0033C0, "Dawnguard.esm")
                             : nullptr;
            const Anchor anchors[] = {
                { "LinkCustom02",   RE::TESForm::LookupByID<RE::BGSKeyword>(0x0005D5E7) },
                { "DLC1LinkSandbox", dlc1 },
            };
            bool any = false;
            for (const auto& a : anchors) {
                ...
                if (auto* anchor = actor->GetLinkedRef(a.kw)) {
                    any = true;
                    const auto apos = anchor->GetPosition();
                    SKSE::log::info("                 -> {} -> {} at {:.0f},{:.0f},{:.0f} cell '{}'",
                                    a.label, Describe(anchor), apos.x, apos.y, apos.z, CellOf(anchor));
                    // HOW FAR THE CHILD IS FROM ITS OWN ANCHOR. This is the
                    // number the whole question turns on: a child standing ON
                    // its anchor is sandboxing in a spot with nothing to use,
                    // and one standing well inside the radius is sandboxing
                    // normally. Without it, "in the right house" and "rooted to
                    // the marker" look identical in a log.
                    const auto dx = pos.x - apos.x, dy = pos.y - apos.y, dz = pos.z - apos.z;
                    SKSE::log::info("                    {:.0f} units from that anchor",
                                    std::sqrt(dx * dx + dy * dy + dz * dz));
                }
            }
            if (!any) {
                SKSE::log::info("                 -> NO anchor on either keyword");
            }
        }
